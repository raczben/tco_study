...
set_output_delay -clock clkB_virt -max [expr $odelay_M] [get_ports {o_native* o_iob* o_odelay*}]
#create the output minimum delay for the data output from the
#FPGA that accounts for all delays specified (tHb + 3.992)
set_output_delay -clock clkB_virt -min [expr $odelay_m] [get_ports {o_native* o_iob* o_odelay*}]
